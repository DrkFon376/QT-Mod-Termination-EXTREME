	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		if(PlayState.isStoryMode){ //Added an extra check here to avoid music playing when in freeplay.
			switch (PlayState.SONG.song.toLowerCase())
			{
				...
				case 'censory-superload' | 'terminate':
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'censory-superload' | 'terminate': //Why are they seperated? ...Erm...  ¯\_(ツ)_/¯
				hasDialog = true;
				box.frames = Paths.getSparrowAtlas('speech_bubble_talking');
				box.animation.addByPrefix('normalOpen', 'Speech Bubble Normal Open', 24, false);
				box.animation.addByPrefix('normal', 'speech bubble normal', 24, false);
		...
		if (PlayState.SONG.song.toLowerCase()=='carefree' || PlayState.SONG.song.toLowerCase()=='careless' || PlayState.SONG.song.toLowerCase()=='censory-superload' || PlayState.SONG.song.toLowerCase()=='terminate' || PlayState.SONG.song.toLowerCase()=='cessation')
		{
		...
		if (FlxG.keys.justPressed.ANY  && dialogueStarted == true)
		{
			...
			if (dialogueList[1] == null && dialogueList[0] != null)
			{
				if (!isEnding)
				{
					...
					else if(PlayState.SONG.song.toLowerCase() == 'censory-superload' || PlayState.SONG.song.toLowerCase()=='terminate' || PlayState.SONG.song.toLowerCase()=='carefree')
